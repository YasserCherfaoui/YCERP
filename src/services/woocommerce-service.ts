...
export const getWooCommerceOrders = async (
    ...
    if (params.franchise_id) url += `&franchise_id=${params.franchise_id}`;
...
export interface DeliveryFeeRow {
  ...
  yalidine_delivery_fee: number | null;
  mismatch: boolean;
  can_correct: boolean;
}

export interface DeliveryFeeCompareResponse {
  rows: DeliveryFeeRow[];
  meta?: {
    total_items: number;
    total_pages: number;
...
export interface DeliveryFeeCorrectResponse {
  requested: number;
  corrected: number;
  skipped_no_live: number;
  errors: number;
...
export const compareDeliveryFeeCorrection = async (body: {
  company_id: number;
  start: string;
  end: string;
  page: number;
  limit: number;
}): Promise<APIResponse<DeliveryFeeCompareResponse>> => {
  const currentToken = localStorage.getItem("token");
  const response = await fetch(
    `${baseUrl}/woocommerce/yalidine-reconciliation/delivery-fee/compare`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(errorData.message || "Failed to load delivery fee comparison");
  }

  return await response.json();
};

export const correctDeliveryFees = async (body: {
  company_id: number;
  order_ids: number[];
}): Promise<APIResponse<DeliveryFeeCorrectResponse>> => {
  const currentToken = localStorage.getItem("token");
  const response = await fetch(
    `${baseUrl}/woocommerce/yalidine-reconciliation/delivery-fee/correct`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(errorData.message || "Failed to correct delivery fees");
  }
