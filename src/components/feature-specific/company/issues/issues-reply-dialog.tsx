...
export default function IssuesReplyDialog({ open, onClose, issue }: IssuesReplyDialogProps) {
  const { register, handleSubmit, reset } = useForm<{ reply: string,issue_ticket_id: number }>({ defaultValues: { reply: "", issue_ticket_id : issue?.id } });
      ...
      <DialogContent>
        ...
        <form
          onSubmit={handleSubmit((data) => {
            createIssueReplyMutation({ ...data, issue_ticket_id: issue?.id ?? 0 });
            handleDialogClose();
          })}
          className="space-y-4"
        >
