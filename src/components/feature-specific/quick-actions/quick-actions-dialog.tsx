...
import { CalendarIcon, Loader2, Search, Zap } from "lucide-react";
...
const locationTypes = ["warehouse", "algiers", "franchise"];

export default function QuickActionsDialog({ open, onOpenChange }: QuickActionsDialogProps) {
  ...
  // Fetch delivery employees for the current company
  const { data: employeesData } = useQuery({
    queryKey: ["delivery-employees", company?.ID],
    queryFn: () => getDeliveryEmployees(company?.ID || 0),
    enabled: !!company?.ID,
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="qr-sales" className="space-y-4">
