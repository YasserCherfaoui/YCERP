  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-1 flex-col", className)}>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 p-3 pt-2">
        ...
        />
        <Composer draft={draft} setDraft={setDraft} onSend={onSend} rows={2} minHClass="min-h-[56px]" textareaClass="text-sm" />
      </div>
    </div>
  );
}
...
function Toolbar({
  ...
  const scrollInner = (
    <div className="flex flex-col gap-2.5 p-2.5">
      {messages.map((m) => (
        <Bubble key={m.uid} message={m} compact isOwn={isOwnMessage(m)} />
      ))}
...
function Composer({
  ...
  return (
    <div className="flex flex-shrink-0 flex-col gap-2 sm:flex-row sm:items-end sm:rounded-2xl sm:border sm:border-border/70 sm:bg-muted/30 sm:p-2">
      ...
      />
      <Button type="button" onClick={onSend} className={cn("rounded-xl sm:shrink-0", rows <= 2 ? "h-9 sm:w-[4.75rem]" : "md:w-32 shrink-0")}>
        Send
      </Button>
    </div>
  );
...
function Bubble({
  ...
  return (
    <div className={cn("flex w-full flex-col", isOwn ? "items-end" : "items-start")}>
      {!isOwn ? (
        <div className={cn("mb-1 max-w-[min(92%,420px)] px-1 text-muted-foreground", compact ? "text-[10px]" : "text-xs")}>
      ...
      >
        <p className={cn("whitespace-pre-wrap break-words leading-relaxed", compact ? "text-xs" : "text-[15px]")}>
          {message.body}
        </p>
        <div
          className={cn(
